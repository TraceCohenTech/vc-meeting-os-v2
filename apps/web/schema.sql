...
-- Full-text search vector (maintained by trigger; generated columns require immutable expressions)
...
-- Maintain memos.fts
CREATE OR REPLACE FUNCTION set_memos_fts()
RETURNS TRIGGER AS $$
BEGIN
  NEW.fts :=
...
CREATE TRIGGER set_memos_fts_trigger
  BEFORE INSERT OR UPDATE OF title, content, summary, tags ON memos
  FOR EACH ROW EXECUTE FUNCTION set_memos_fts();
